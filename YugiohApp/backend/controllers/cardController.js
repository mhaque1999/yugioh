...
// async function fetchCardDataFromAPI(params) { //dont need this to run everytime
//   try {
//     const response = await axios.get(`${YGOPRO_API_BASE_URL}/cardinfo.php`, {
//       params: params,
//     });
...
// const getAllCardsFromTable = async (req, res) => {
//   try {
...
// cron.schedule('0 0 * * *', async () => { //scheudule adding cards sunday midnight. maybe add some notification to let user know
//   try {
...
//       let card = await Card.findOne({ where: { name: apiCardData.name } });
//       //
//       const imgurImg = await axios.post('https://api.imgur.com/3/image',
//         { Authorization: '6ebd0b63a521a312263f1a5e29adfd3bfc2eba4f',
//           image: apiCardData.card_images[0].image_url,
//           type:"url",
//           title: apiCardData.name,
//           description:apiCardData.desc});
...
//     const cardData = await fetchAllCardDataFromAPI();
//     const storedCards = [];
//     const rateLimit = 50; // Number of requests per minute
//     const delay = 60000 / rateLimit; // Delay in milliseconds between requests
...
//         requestCount++;
//       } catch (imgurError) {
//         console.error(`Error uploading image for card: ${apiCardData.name}`, imgurError);
...
//     for (const apiCardData of cardData) {
//       try {
//         // Ensure a delay to prevent exceeding rate limit
//         await new Promise(resolve => setTimeout(resolve, delay));

//         const imgurImg = await limiter.schedule(()=> axios.post('https://api.imgur.com/3/image', {
//           headers: {
//             Authorization: '6ebd0b63a521a312263f1a5e29adfd3bfc2eba4f',
//           },
//           image: apiCardData.card_images[0].image_url,
...
//             ban_ocg: apiCardData.ban_ocg || null,
//             ban_goat: apiCardData.ban_goat || null,
//             image_url: imgurImg.data.data.link,
//             image_url_small: apiCardData.card_images[0].image_url_small,
//             cardmarket_price: apiCardData.card_prices[0].cardmarket_price || null,
//             tcgplayer_price: apiCardData.card_prices[0].tcgplayer_price || null,
//             ebay_price: apiCardData.card_prices[0].ebay_price || null,
...
//           card.ban_ocg = apiCardData.ban_ocg || null;
//           card.ban_goat = apiCardData.ban_goat || null;
//           card.image_url = imgurImg.data.data.link;
//           card.image_url_small = apiCardData.card_images[0].image_url_small;
//           card.cardmarket_price = apiCardData.card_prices[0].cardmarket_price || null;
//           card.tcgplayer_price = apiCardData.card_prices[0].tcgplayer_price || null;
//           card.ebay_price = apiCardData.card_prices[0].ebay_price || null;
...
async function fetchAndStoreAllCards(req, res) {
  try {
    ...
    for (const apiCardData of cardData) {
      try {
        apiCardData.card_images[0].image_url;
        let card = await Card.findOne({ where: { name: apiCardData.name } });
        ...
        console.error(`Error uploading image for card: ${apiCardData.name}`, error);
      }
    }
...
//fetchAndStoreAllCards().catch(error => console.error('Failed to fetch and store cards:', error));
