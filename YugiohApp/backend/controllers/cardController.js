...
//     // Check the count for each card and filter out those exceeding the limit
...
//     // Bulk create DeckCard entries for valid cards
//     if (validCardEntries.length > 0) {
//       await DeckCard.bulkCreate(validCardEntries);
//     }
...
//     // Use a transaction to handle concurrent requests and ensure atomicity
//     const result = await sequelize.transaction(async (transaction) => {
//       // Check current counts and filter card IDs that exceed the limit
//       const cardCounts = await DeckCard.findAll({
//         attributes: ['cardId', [sequelize.fn('COUNT', sequelize.col('cardId')), 'count']],
//         where: { deckId },
//         group: 'cardId',
//         transaction
//       });

//       const cardCountMap = cardCounts.reduce((map, { cardId, count }) => {
//         map[cardId] = count;
//         return map;
//       }, {});
...
//         if (existingCount < 3) {
//           validCardEntries.push({ deckId, cardId });
//         } else {
//           invalidCardIds.push(cardId);
//         }
//       }
...
//       const cardCounts = await DeckCard.findAll({
//         attributes: ['cardId', [sequelize.fn('COUNT', sequelize.col('card_id')), 'count']],
//         where: { deckId },
//         group: 'cardId',
//         transaction
//       });
...
//           where: {
//             deckId,
//             cardId: validCardEntries.map(entry => entry.cardId)
//           },
//           transaction
//         });
//       }
...
async function addCardsToDeck(req, res) {
  ...
  try {
    const result = await sequelize.transaction(async (transaction) => {
      ...
      const cardCounts = await DeckCard.findAll({
        // attributes: [
        //   "card_id",  // Correct column name
        //   [sequelize.fn('COUNT', sequelize.col('card_id')), 'count']
        // ],
        // where: { deck_id: deckId },  // Correct column name
        // group: 'card_isd',  // Correct column name
        // transaction
        attributes: [
